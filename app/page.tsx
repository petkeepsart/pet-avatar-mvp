...
const GUARANTEES = [
  { icon: "🔒", title: "Secure Stripe Checkout", text: "Payments are processed securely by Stripe — the same system used by Amazon and Apple. We never see your card details." },
  { icon: "⚡", title: "Instant Digital Delivery", text: "Files are generated and emailed to you within minutes of uploading your photo. No shipping, no waiting days." },
  { icon: "🎨", title: "You Personalize Everything", text: "After payment, choose your text, font, colors and layout before the final files are created — nothing is locked in until you're happy." },
  { icon: "📧", title: "Files Sent to Your Email", text: "Your personalized PDF and ZIP files are delivered directly to the email used at checkout, ready to download whenever you like." },
  { icon: "🐾", title: "Any Pet, Any Occasion", text: "Dogs, cats, rabbits, birds — any beloved pet. Designs work for birthdays, memorials, gifts and everyday fun." },
  { icon: "💬", title: "Fast Delivery · Peace of Mind", text: "Instant digital delivery — no waiting. If you have any questions, we're always here to help." },
];
      ...
      <div className="relative mx-auto flex max-w-[1680px] items-center justify-between gap-3 px-3 py-2.5">
        ...
        <div className="pointer-events-none absolute left-1/2 hidden -translate-x-[97%] lg:flex">
          <div className="pointer-events-auto flex items-center gap-5">
            <a href="#pricing" className="whitespace-nowrap text-[16px] font-bold text-[#4A3428] hover:opacity-70">Bundle</a>
            <a href="#pricing" className="whitespace-nowrap text-[16px] font-bold text-[#4A3428] hover:opacity-70">Pricing</a>
            <a href="#guide" className="whitespace-nowrap text-[16px] font-bold text-[#4A3428] hover:opacity-70">FAQ</a>
            <a href="#examples" className="whitespace-nowrap text-[16px] font-bold text-[#4A3428] hover:opacity-70">View Examples</a>
          </div>
        </div>
        ...
        <div className="flex flex-col items-center">
          <div className="w-full max-w-[1480px] text-center">
            <div className="inline-flex rounded-full border border-[#d8c7b6] bg-white/90 px-7 py-3 text-[16px] font-extrabold uppercase tracking-[0.12em] text-[#7B5B47] shadow-sm">
              Personalized digital keepsakes from your own pet or pet + owner photo
            </div>
            <h1 className="mx-auto mt-6 max-w-[1200px] text-center font-serif font-black leading-[1.02] tracking-[-0.04em] text-[#23150F] text-[clamp(2rem,3.6vw,4rem)]">
              1 Photo → 3 Personalized Keepsakes
            </h1>
            <p className="mx-auto mt-5 max-w-[1080px] px-2 text-[clamp(1rem,1.2vw,1.3rem)] leading-[1.55] text-stone-700">
              Turn your pet or pet + owner photo into a printable coloring page, a keepsake certificate, and a 12-avatar pack — ready to download, print, gift, and treasure.
            </p>
            <div className="mt-6 flex flex-wrap items-center justify-center gap-3">
              <HeroPill>Made from your own photo</HeroPill>
              <HeroPill>Pet-only or pet + owner photos</HeroPill>
              <HeroPill>Custom, Birthday or memorial-ready</HeroPill>
            ...
            <div className="flex flex-col items-center">
              <div className="w-full max-w-[420px] rounded-[30px] border border-[#d9cbbc] bg-white p-7 shadow-[0_18px_34px_rgba(84,58,39,0.10)]">
                <div className="mb-4 flex items-center justify-between gap-3">
                  <span className="rounded-full bg-[#F7EBDD] px-4 py-1.5 text-[13px] font-extrabold uppercase tracking-[0.12em] text-[#8A5A3D]">Sample photo</span>
                ...
                <div className="mt-6 flex flex-col gap-3">
                  <a href="#pricing" className="w-full rounded-full bg-[linear-gradient(180deg,#D98962_0%,#C86C43_100%)] px-5 py-4 text-center text-[20px] font-extrabold text-white shadow-[0_12px_22px_rgba(157,97,65,0.22)] transition hover:brightness-95">Choose Your Bundle</a>
                  <a href="#examples" className="w-full rounded-full border border-[#baa692] px-5 py-4 text-center text-[20px] font-bold text-[#5b4334] transition hover:bg-[#faf3eb]">View Real Examples</a>
                  <div className="mt-2 rounded-[18px] border border-[#eadfd2] bg-[#fffaf4] px-4 py-4 text-center text-[16px] leading-7 text-[#7B6658]">
                    <div>You'll upload your photo after payment.</div>
                    <div className="mt-2 font-semibold text-[#6E5546]">Files will be sent to the same email entered at Stripe checkout.</div>
                    <div className="mt-1 font-semibold text-[#6E5546]">That delivery email cannot be changed later on the customize page.</div>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-6 w-full max-w-[420px] rounded-[28px] border border-[#e1d3c5] bg-white p-6 shadow-[0_10px_22px_rgba(84,58,39,0.06)]">
                <div className="text-[24px] font-black text-[#4B3427]">Billing location</div>
                <div className="mt-3 rounded-[16px] border border-[#e6d8ca] bg-[#fffaf4] px-4 py-3 text-[14px] leading-6 text-[#7B6658]">
                  Canada customers must select a province or territory. Québec orders are not accepted.
                </div>
                <div className="mt-5 space-y-4">
                  <div>
                    <label className="mb-2 block text-[14px] font-bold text-[#6B5345]">Country</label>
                  ...
                  <div>
                    <label className="mb-2 block text-[14px] font-bold text-[#6B5345]">{isCanada ? "Province / Territory" : "State / Province"}</label>
                    {isCanada ? (
                  ...
                  {missingProvince && <div className="rounded-[16px] border border-amber-200 bg-amber-50 px-4 py-3 text-[15px] font-semibold text-amber-700">Please select a province to continue.</div>}
                  {isQuebecBlocked && <div className="rounded-[16px] border border-red-200 bg-red-50 px-4 py-3 text-[15px] font-semibold text-red-700">Sorry, this service is not currently available to customers located in Québec.</div>}
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col items-center xl:items-start">
              <div className="w-full max-w-[980px] rounded-[32px] border border-[#e1d3c5] bg-[linear-gradient(180deg,#fffaf3_0%,#f8efe5_100%)] p-5 shadow-[0_16px_28px_rgba(84,58,39,0.06)]">
                <div className="mb-5 flex flex-wrap items-center justify-between gap-3 px-2">
                  <span className="rounded-full bg-white px-5 py-2 text-[14px] font-extrabold uppercase tracking-[0.12em] text-[#8A5A3D] shadow-sm">Live preview mockup</span>
                  <span className="text-[16px] font-semibold text-[#7B6658]">1 upload • 3 product styles</span>
                </div>
                <div className="grid gap-6 md:grid-cols-3">
                  <div className="relative flex min-h-[440px] flex-col items-center gap-3 rounded-[28px] border border-[#ead9c8] bg-white p-5 shadow-sm">
                    ...
                    <div className="rounded-full bg-[#5b5551] px-3 py-1 text-[10px] font-extrabold uppercase tracking-[0.14em] text-white">AI Preview Mockup</div>
                  ...
                  <div className="relative flex min-h-[440px] flex-col items-center gap-3 rounded-[28px] border border-[#ead9c8] bg-white p-5 shadow-sm">
                    ...
                    <div className="-mt-6 px-2 flex flex-col items-center text-center">
                      <div className="whitespace-nowrap text-[12px] font-bold leading-none text-[#8A5A3D]">
                        Personalized layout created after
                      </div>
                      <div className="mt-1 text-[12px] font-semibold leading-tight text-[#8A5A3D]">
                        payment. Custom, Birthday, or Memorial
                      </div>
                    </div>
                  </div>
                  <div className="relative flex min-h-[440px] flex-col items-center gap-3 rounded-[28px] border border-[#ead9c8] bg-white p-5 shadow-sm">
              ...
              <div id="pricing" className="mt-8 grid w-full max-w-[980px] gap-5 sm:grid-cols-2 xl:grid-cols-4">
                <button type="button" onClick={() => startCheckout("bundle")} disabled={checkoutDisabled} className="relative flex min-h-[280px] scale-[1.02] flex-col items-center justify-center overflow-hidden rounded-[32px] border border-[#cfb39b] bg-[linear-gradient(180deg,#D98962_0%,#C86C43_100%)] px-5 py-7 text-center text-white shadow-[0_20px_30px_rgba(151,90,59,0.26)] transition hover:brightness-95 disabled:cursor-not-allowed disabled:opacity-70">
                  ...
                  <div className="mt-4 rounded-full bg-white/15 px-4 py-1.5 text-[14px] font-bold">{loadingPlan === "bundle" ? "Redirecting..." : pricingText.bundleSave}</div>
                  <div className="mt-4 flex flex-col items-center text-[12px] font-semibold leading-5 text-white/90"><span>SAME EMAIL AS</span><span>STRIPE CHECKOUT</span></div>
                </button>
                ...
                <PricingCard title={pricingText.keepsakeTitle} price="$12.99" subtitle={pricingText.singleSub} onClick={() => startCheckout("keepsake")} isLoading={loadingPlan === "keepsake"} isDisabled={checkoutDisabled}
                  extra={<div className="mt-2 text-[15px] leading-[1.35] text-[#8D6A50]"><div>Custom /</div><div>Happy Birthday OR</div><div>In Loving Memory</div></div>}
                />
              ...
              <div className="mt-6 flex w-full max-w-[980px] flex-wrap items-center justify-center gap-3">
                ...
                <TrustPill>Same email as checkout</TrustPill>
                <TrustPill>Cannot change later</TrustPill>
              </div>
            </div>
          </div>
        </div>
...
function PricingCard({ title, price, subtitle, extra, onClick, isLoading, isDisabled }: { title: string; price: string; subtitle: string; extra?: React.ReactNode; onClick?: () => void; isLoading?: boolean; isDisabled?: boolean }) {
  return (
    <button type="button" onClick={onClick} disabled={isDisabled} className="flex min-h-[280px] flex-col items-center justify-center rounded-[32px] border border-[#c8b9ab] bg-white px-5 py-6 text-center shadow-[0_10px_22px_rgba(84,58,39,0.08)] transition hover:bg-[#faf3eb] disabled:cursor-not-allowed disabled:opacity-70">
      <div className="text-[19px] leading-tight font-black text-[#4B3427]">{isLoading ? "Redirecting..." : title}</div>
      {!isLoading && extra}
      ...
      <div className="mt-3 text-[13px] font-semibold leading-5 text-[#8D6A50]">Sent to checkout email only</div>
    </button>
  );
}
  ...
  return (
    <section className="border-t border-[#eadfd2] bg-[#fbf8f2] px-6 py-16 md:px-10 xl:px-12">
      <div className="mx-auto max-w-[1380px]">
        <h2 className="text-center font-serif text-[42px] font-black tracking-[-0.04em] text-[#24150F] md:text-[56px]">How It Works</h2>
        <p className="mx-auto mt-4 max-w-[980px] text-center text-[20px] leading-[1.65] text-stone-600 md:text-[22px]">One clear pet or pet + owner photo becomes a printable keepsake bundle in a few simple steps.</p>
        <div className="mt-10 grid gap-8 md:grid-cols-3">
          <HowCard number="1" icon={<ShoppingBag className="h-10 w-10" strokeWidth={2.4} />} title="Choose your keepsake" text="Pick the bundle or single keepsake you want and complete payment first." />
          <HowCard number="2" icon={<PawPrint className="h-10 w-10" strokeWidth={2.4} />} title="Upload after payment" text="After checkout, upload one clear pet or pet + owner photo and add details if needed." />
          <HowCard number="3" icon={<Sparkles className="h-10 w-10" strokeWidth={2.4} />} title="Receive your files" text="We generate your personalized digital keepsakes for download and send them to your checkout email." />
        </div>
      </div>
    </section>
  );
    ...
    <section id="examples" className="border-t border-[#eadfd2] bg-[linear-gradient(180deg,#fffdfa_0%,#fff7ef_100%)] px-6 py-16 md:px-10 xl:px-12">
      <div className="mx-auto max-w-[1500px]">
        <div className="text-center">
          <h2 className="font-serif text-[42px] font-black tracking-[-0.04em] text-[#24150F] md:text-[56px]">Real Product Style Preview</h2>
          <p className="mx-auto mt-4 max-w-[1040px] text-[20px] leading-[1.65] text-stone-600 md:text-[22px]">
            <span className="block">See how one uploaded pet or pet + owner photo can become a printable coloring page,</span>
            <span className="block">a personalized keepsake certificate, and a fun 12-avatar pack.</span>
          </p>
        </div>
        <div className="mx-auto mt-10 max-w-[1450px] rounded-[36px] border border-[#e8ddd1] bg-[linear-gradient(180deg,rgba(255,255,255,0.9)_0%,rgba(255,251,246,0.98)_100%)] p-6 shadow-[0_18px_40px_rgba(84,58,39,0.08)] md:p-8 xl:p-10">
          <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-4">
            <ExampleFlowCard img="/images/avatar_example.jpg" title="Original Photo" badge="Original" description="The pet or pet + owner photo you upload" badgeTone="gold" />
            <ExampleFlowCard img="/images/coloring_result.jpg" title="Coloring Page" badge="AI Preview" description="Printable black-and-white line art" badgeTone="gold" />
            <ExampleFlowCard img="/images/keepsake-certificate.png" title="Keepsake Certificate" badge="AI Preview" description="Custom, Birthday or memorial-ready keepsake" badgeTone="gold" />
            <ExampleFlowCard img="/images/avatar_pack_preview.png" title="Avatar Pack" badge="AI Preview" description="12 unique shareable avatar styles" badgeTone="gold" contain />
          </div>
        </div>
      </div>
    </section>
...
function ReviewsSection() {
  return (
    <section className="border-t border-[#eadfd2] bg-[#fffaf5] px-6 py-16 md:px-10 xl:px-12">
      <div className="mx-auto max-w-[1500px]">
        <div className="text-center">
          <h2 className="font-serif text-[42px] font-black tracking-[-0.04em] text-[#24150F] md:text-[56px]">Why Pet Families Choose Us</h2>
          <p className="mx-auto mt-4 max-w-[780px] text-[20px] leading-[1.65] text-stone-600 md:text-[22px]">Everything is designed to make your experience simple, safe and special.</p>
        </div>
...
function PhotoGuideSection() {
  return (
    <section id="guide" className="border-t border-[#eadfd2] bg-[#fffaf4] px-6 py-16 md:px-10 xl:px-12">
      <div className="mx-auto max-w-[1500px]">
        <div className="text-center">
          <h2 className="font-serif text-[42px] font-black tracking-[-0.04em] text-[#24150F] md:text-[56px]">How to Choose the Best Photo</h2>
          <p className="mx-auto mt-4 max-w-[980px] text-[20px] leading-[1.65] text-stone-600 md:text-[22px]">A clear, bright photo gives the best keepsake results.</p>
        </div>
        <div className="mt-10 grid gap-8 lg:grid-cols-2">
          <GuidePanel good title="✓ Good" bullets={["Realistic lighting on the pet face", "Face clearly visible and centered", "Pet fills enough of the frame"]} images={["/images/good_1_bird_girl.png", "/images/good_2_rabbit.png", "/images/good_3_hedgehog_girl.png"]} />
          <GuidePanel title="✕ Avoid" bullets={["Blurry or out of focus photo", "Too dark or heavy shadows", "Face cropped or turned away"]} images={["/images/avoid_1_blurry_cat.png", "/images/avoid_2_dark_cat.png", "/images/avoid_3_turned_away_bird.png"]} />
        </div>
      </div>
    </section>
  );
  ...
  return (
    <section className="border-t border-[#eadfd2] bg-[linear-gradient(180deg,#fff8f0_0%,#f8eee3_100%)] px-6 py-16 md:px-10 xl:px-12">
      <div className="mx-auto flex max-w-[1320px] flex-col items-center text-center">
        <h2 className="mx-auto max-w-[1180px] text-center font-serif text-[42px] font-black tracking-[-0.04em] text-[#24150F] md:text-[56px]">Ready to turn your pet or pet + owner photo into something worth keeping?</h2>
        <p className="mx-auto mt-4 max-w-[1280px] text-center text-[20px] leading-[1.65] text-stone-600 md:text-[22px]">Choose your bundle first. You'll upload your photo after payment.</p>
        <p className="mx-auto mt-2 max-w-[980px] text-center text-[17px] leading-8 text-[#7B6658]">Your final files will be delivered to the same email entered at checkout. No alternate delivery email can be selected later.</p>
        <div className="mt-8">
          <a href="#pricing" className="inline-flex rounded-full bg-[linear-gradient(180deg,#D98962_0%,#C86C43_100%)] px-10 py-4 text-[20px] font-extrabold text-white shadow-[0_14px_24px_rgba(157,97,65,0.24)] transition hover:brightness-95">Choose Your Bundle</a>
        </div>
        <div className="mt-4 text-[18px] font-medium leading-8 text-[#7B6658]">Digital delivery • No shipping • Great for gifts and memory</div>
      </div>
    </section>
  );
}
        ...
        <div className="flex flex-wrap items-center justify-center gap-4">
          {socialItems.map((item) => (
            <button key={item.key} type="button" onClick={item.key === "link" ? copyLink : undefined} aria-label={item.alt} className="relative transition hover:opacity-75">
              ...
              {item.key === "link" && linkCopied && <span className="absolute -top-7 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-full bg-white px-2 py-1 text-[10px] font-bold text-[#b38a3d] shadow">COPIED</span>}
            </button>
          ))}
        </div>
...
function ContactModal({ name, setName, supportSubject, setSupportSubject, message, setMessage, close }: { name: string; setName: React.Dispatch<React.SetStateAction<string>>; supportSubject: string; setSupportSubject: React.Dispatch<React.SetStateAction<string>>; message: string; setMessage: React.Dispatch<React.SetStateAction<string>>; close: () => void }) {
  const buildMailto = () => {
    const finalSubject = supportSubject || "Support Request";
    const bodyLines = [name ? `Name: ${name}` : "", "", message || "How can we help?"].filter(Boolean);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/45 px-4 backdrop-blur-sm" onClick={close}>
      <div className="relative w-full max-w-[560px] rounded-[24px] border border-[#e3d8cb] bg-[#FAF6F0] p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="mt-6 space-y-4">
          ...
          <textarea rows={4} placeholder="How can we help?" value={message} onChange={(e) => setMessage(e.target.value)} className="w-full rounded-xl border border-[#d8cdbf] bg-white px-4 py-3 outline-none" />
          <button type="button" className="w-full rounded-[12px] bg-[#3D2B1F] px-6 py-4 text-base font-bold text-white shadow-md transition hover:bg-[#2A1D15]" onClick={() => { window.location.href = buildMailto(); }}>Open Email App</button>
        </div>
      </div>
    </div>
  );
